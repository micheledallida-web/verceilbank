...
alter table public.user_profile
  ...
  -- The seven-digit code an account was opened with, back when sign-up was
  -- invite-only. That gate is gone and nothing issues codes any more, so this
  -- is history: it holds the codes accounts were actually opened with, and is
  -- kept because provision_user still writes it and dropping it would break
  -- every sign-up. New rows land here empty.
...
begin
  begin
    ...
    insert into public.user_profile (
      ...
      nullif(meta->>'postal_code', ''),
      p_email,
      -- Which code opened this account, back when sign-up was invite-only.
      -- Nothing collects a code any more, so this is null on every new
      -- account; it is kept because it is the only record of which invitation
      -- opened the accounts that were created while the gate ran.
...
select 'row level security', c.relname,
       case when not c.relrowsecurity then 'EXPOSED — rls off'
            ...
            -- The one table that is MEANT to have no policy. Deny-all is the
            -- design, not an oversight: raw exchange payloads, written by the
            -- webhook with the service key. Without this it reads as a fault
            -- every time anybody runs the file, which is how a real fault two
            -- lines below gets skimmed past.
            when c.relname = 'deposit_events'
